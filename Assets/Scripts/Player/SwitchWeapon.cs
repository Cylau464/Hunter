...
public class SwitchWeapon : MonoBehaviour
{
    ...
    void Update()
    {
        if (curCooldown <= Time.time && input.switchWeapon && !playerMovement.isHurt && !playerMovement.isAttacking && !playerMovement.isDead && !playerMovement.isCast)
            Switch();
    }
    ...
    void Switch()
    {
        ...
        if (mainWeapon.GetComponent<WeaponAttributes>().weaponAttackType == WeaponAttackType.Range)
            mainWeapon.GetComponent<Aim>().SetCursorDefault();

        if (secondWeapon.GetComponent<WeaponAttributes>().weaponAttackType == WeaponAttackType.Range)
            secondWeapon.GetComponent<Aim>().SetCursorTexture();
